...
typedef int64_t       stype;   /* sum of profit or weight  */
...
double combo_solve(double * p, double * w, int * x, int64_t W, double lb, double ub, int n)
{
  ...
  ////////// Adjustment of the problem to change doubles into int64_t //////////
  double sumw = 0;
  double sump = 0;
  for (int i=0; i<n; i++)
  {
  ...
  //int64_t int64_t_max_val = 9223372036854775807;
  //double mult_max_w = (double)int64_t_max_val/sumw;
  //double mult_max_p = (double)int64_t_max_val/sump;
  //double mult_max = mult_max_w;
  //if(mult_max_p < mult_max){
  //  mult_max = mult_max_p;
  //}
  int mult = 1048576; //2^20 (20 bits of precision)
  //printf("mult_max = %f\n", mult_max);
  //if(mult_max < mult){
  //  mult = (int)round(mult_max/10);
  //}
  //printf("mult = %d\n", mult);
  ////////// Adjustment of the problem to change doubles into int64_t //////////
  ...
  for(int i=0;i<n;i++)
  {
    items[i].p = (int64_t)round(mult*p[i]);
    items[i].w = (int64_t)round(mult*w[i]);
    items[i].x = 0;
    x[i] = 0;
    items[i].id = i;
  }
  ...
  int64_t lb_combo = (int64_t)(floor(lb*mult));
  int64_t ub_combo = (int64_t)(ceil(ub*mult));

  z = combo(items, &items[n-1], W*mult, lb_combo, ub_combo, TRUE, TRUE);

  for(int i=0 ; i<n ; i++){
    x[ items[i].id ] = items[i].x;
  }
